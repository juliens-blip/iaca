# Skill: Monitoring Quota & Handoff Codex/AMP

## Vue d'ensemble

Cette skill permet à Codex d'orchestrer une équipe de LLMs, de **surveiller son propre quota de session**, et de **transférer automatiquement l'orchestration à AMP** quand le quota atteint 93%.

**Capacités couvertes :**
1. Monitoring du quota de session Codex
2. Handoff automatique vers AMP à 93%
3. Reprise de l'orchestration par AMP
4. Retour à Codex après reset du quota
...
Le quota de session Codex est affiché dans le **footer** de la fenêtre tmux :

```
You've used 93% of your session limit · resets 2pm (Europe/Paris)
```

**Important** : Cette info est côté serveur du fournisseur LLM, pas stockée localement.

## A.2 Vérification manuelle du quota

```bash
# Capturer le footer et extraire le pourcentage
tmux capture-pane -t $SESSION:codex -p | grep -oE "used [0-9]+%" | grep -oE "[0-9]+" | tail -1
```

## A.3 Script de vérification : `check-session-quota.sh`

```bash
#!/bin/bash
# ============================================================================
# check-session-quota.sh - Vérifie le quota de session Codex
# ============================================================================
# Usage: ./check-session-quota.sh [session] [window] [alert_threshold]
# ============================================================================
...
```bash
# Vérification simple
./check-session-quota.sh

# Avec paramètres personnalisés
./check-session-quota.sh orchestration-moana codex 90

# Dans un script, récupérer le quota
QUOTA=$(./check-session-quota.sh 2>/dev/null | grep "^QUOTA=" | cut -d= -f2)
...
LOG_FILE="/tmp/session_quota_watchdog.log"
QUOTA_FILE="/tmp/session_current_quota"
ALERT_TRIGGERED="/tmp/session_quota_alert_triggered"
...
send_alert() {
    ...
    notify-send -u critical "Session Quota" "Session à ${quota}%!" 2>/dev/null

    # Notifier AMP
    tmux send-keys -t "$SESSION:amp" \
        "ALERTE: Codex est à ${quota}%. Prépare-toi pour le handoff." Enter
}
...
# Voir les logs
tail -f /tmp/session_quota_watchdog.log

# Lire le quota actuel
cat /tmp/session_current_quota
```
...
Codex doit créer `HANDOFF_TO_AMP.md` dans le projet :

```markdown
# HANDOFF ORCHESTRATION: Codex -> AMP

**Date**: [timestamp]
**Raison**: Quota session Codex à XX%
...
# Windows: 4=AMP, 2=Antigravity, 5=Codex
```

### Étape 3 : Envoyer le message de handoff à AMP

```bash
tmux send-keys -t $SESSION:4 "HANDOFF ORCHESTRATION: Tu prends le relais comme orchestrateur.
...
#!/bin/bash
# ============================================================================
# auto_handoff_to_amp.sh - Handoff automatique Codex -> AMP
# ============================================================================
...
echo "======= AUTO HANDOFF CODEX -> AMP ======="

# 1. Vérifier quota
QUOTA=$(tmux capture-pane -t "$SESSION:codex" -p | grep -oE "used [0-9]+%" | grep -oE "[0-9]+" | tail -1)
...
tmux send-keys -t "$SESSION:4" "HANDOFF: Tu prends le relais. Lis: cat $HANDOFF_FILE puis cat $PROJECT_DIR/CLAUDE.md" Enter
...
## D.3 Vérifier si Codex peut reprendre

```bash
# Vérifier le quota de Codex
./check-session-quota.sh

# Si quota < 50%, Codex peut reprendre
# Le quota se reset généralement à 14h (Europe/Paris)
```

## D.4 Rendre le contrôle à Codex

```bash
# 1. Créer fichier de retour
cat > HANDOFF_TO_CODEX.md << 'EOF'
# HANDOFF RETOUR: AMP -> Codex

**Date**: $(date)
**Raison**: Quota Codex réinitialisé
...
# 2. Notifier Codex
tmux send-keys -t $SESSION:1 "RETOUR: Tu reprends l'orchestration. Lis HANDOFF_TO_CODEX.md" Enter
```
...
tmux capture-pane -t $SESSION:codex -p | grep -oE "used [0-9]+%"
...
|---------|-------------|
| `CLAUDE.md` | Instructions du projet |
| `HANDOFF_TO_AMP.md` | Fichier de handoff Codex->AMP |
| `HANDOFF_TO_CODEX.md` | Fichier de retour AMP->Codex |
| `/tmp/session_current_quota` | Quota actuel (si watchdog) |
| `/tmp/session_quota_watchdog.log` | Log du watchdog |
