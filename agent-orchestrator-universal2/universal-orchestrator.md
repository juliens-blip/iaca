...
1) **Codex orchestrator high (w4)**: strategic decomposition, hardest decisions, final arbitration.
2) **AMP (w2)**: main implementation worker for complex/multi-file tasks.
3) **Antigravity (w5)**: complex backup worker and second-opinion execution path.
4) **Codex fast (w1)**: quick tasks, checks, refactors, and lightweight fixes.
