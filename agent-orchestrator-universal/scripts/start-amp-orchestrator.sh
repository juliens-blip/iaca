...
if ! tmux list-windows -t "$SESSION" | grep -q "^${WINDOW}:"; then
  tmux new-window -t "$SESSION:$WINDOW" -n amp -c "$PROJECT_DIR"
fi

tmux send-keys -t "$TARGET" C-c
sleep 0.2
tmux send-keys -t "$TARGET" "cd \"$PROJECT_DIR\" && amp -m large --dangerously-allow-all" Enter

for _ in $(seq 1 40); do
  if tmux list-panes -t "$TARGET" -F "#{pane_current_command}" 2>/dev/null | grep -qE "^(amp|node)$"; then
    break
  fi
  sleep 0.5
done
...
if ! tmux list-panes -t "$TARGET" -F "#{pane_current_command}" 2>/dev/null | grep -qE "^(amp|node)$"; then
  echo "ERROR: amp process not confirmed in $TARGET"
...
Tu opères depuis la fenêtre '${WINDOW}' (AMP, orchestrateur).
Hiérarchie à appliquer: Opus (w3) > Sonnet (w2) > Claude w8 > AMP (w4) > Codex (w5) > Haiku (w1).
Lis CLAUDE.md, vérifie l'état des workers, puis exécute immédiatement le Session Bootstrap Protocol de l'agent chargé.
EOF
)

# Paste/send full multi-line prompt to amp.
