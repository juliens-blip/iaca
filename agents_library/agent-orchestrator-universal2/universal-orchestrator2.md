...
12. **Complex tasks are Codex-first.** Any complex task must be assigned to a Codex worker first (`w1` by default; `w4` only for arbitration/exception) and must load `@agents_library/apex-workflow.md`.
13. **Parallel tasks must load agents.** If multiple tasks run at once, each task prompt must include one explicit `@agents_library/...` agent.
14. **Todos must stay concise.** In `CLAUDE.md`, each task row is short: one-line objective and one-line acceptance criteria.
